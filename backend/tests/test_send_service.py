...
def _seed(app, *, email='client@example.com', phone='9876543210', status='draft'):
    with app.app_context():
        ...
        firm = FirmDetails(user_id=user.id, firm_id=tenant.id, firm_name='Acme',
                           firm_address='X', firm_email='acme@firm.com', currency='INR')
        db.session.add(firm)
        client = Client(firm_id=tenant.id, created_by_user_id=user.id,
                        name='Rao', email=email, phone=phone)
...
def test_edited_body_still_gets_link_injected(app):
    ...
    with app.app_context():
        ...
        assert f'https://app/i/{inv.created_by_user_id}/{inv.id}/' in body
...
def test_cc_forwarded_to_transport(app):
    _seed(app)
    with app.app_context():
        inv = Invoice.query.first()
        transport = FakeTransport()
        send_invoice(inv, FirmDetails.query.first(), Client.query.first(), 'email',
                     pdf_bytes=b'%PDF', transport=transport, base_url='https://app',
                     cc='acme@firm.com')
        assert transport.sent['cc'] == 'acme@firm.com'


def test_cc_skipped_when_equals_recipient(app):
    _seed(app, email='acme@firm.com')  # client email same as cc
    with app.app_context():
        inv = Invoice.query.first()
        transport = FakeTransport()
        send_invoice(inv, FirmDetails.query.first(), Client.query.first(), 'email',
                     pdf_bytes=b'%PDF', transport=transport, base_url='https://app',
                     cc='acme@firm.com')
        assert transport.sent['cc'] is None
